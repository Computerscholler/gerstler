...
func rank(results map[Id]bool, queries []string, currentSearchResults map[string]string) []SearchResult {
	...
	for recordID, _ := range results {
		...
		unsortedResults[i] = recordRank{result: SearchResult{Title: record.Title, Link: record.Link, Content: content,Provider: providerType,Matches: numMatches}, score: score}
		i += 1
	}
